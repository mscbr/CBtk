...
class BinanceFuturesClient:
    def __init__(self, testnet, public_key, secret_key):
        if testnet:
    ...
    def get_historical_candles(self, symbol, interval):
        ...
        if raw_candles is not None:
            for c in raw_candles:
                candles.append([c[0], float(c[1]), float(c[2]),
    ...
    def start_ws(self):
        self.ws = websocket.WebSocketApp(
            self.wss_url, on_open=self.on_open, on_close=self.on_close, on_error=self.on_error, on_message=self.on_message)
