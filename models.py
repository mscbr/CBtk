...
BITMEX_MULTIPLIER = 0.00000001  # SATOSHI
...
def tick_to_decimals(tick_size: float) -> int:
    # i.e. "{0:.8f}".format(0.0001) => 0.00010000
    ...
    if len(split_tick) > 1:
        return len(split_tick[1])  # "0.001" -> 3
    else:
...
class Contract:
    def __init__(self, contract_info, exchange):
        if exchange == "binance":
            ...
            self.lot_size = 1 / pow(10, contract_info['quantityPrecison'])
